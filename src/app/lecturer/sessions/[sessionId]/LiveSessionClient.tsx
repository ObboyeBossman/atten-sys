...
function StatusBadge({
  ...
  return (
    ...
    >
      {s.label}
      {isManual && (
        <span style={{ opacity: 0.7, fontSize: 9 }}>MANUAL</span>
      )}
    </span>
  );
}
...
export function LiveSessionClient({
  ...
  // Elapsed timer — updates every 30 seconds
  useEffect(() => {
    const id = setInterval(() => setElapsed(elapsedStr(startedAt)), 30_000);
    return () => clearInterval(id);
  }, [startedAt]);

  // Realtime subscription on attendance table
  useEffect(() => {
    const supabase = createSupabaseBrowserClient();
    const channel = supabase
      .channel(`lecturer-session-${sessionId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  async function handleMark(
    ...
    if ("error" in res) {
      ...
      setMarkMsg({
        id: entry.studentId,
        msg: `Marked ${entry.studentName} as ${status}.`,
      });
  ...
  function handleEndConfirm() {
    ...
    startTransition(async () => {
      ...
      if ("error" in res) {
        ...
        router.push(`/lecturer/sessions/${sessionId}/attendance`);
      }
    });
  }
  ...
  // Sort: checked-in first (present→late→null→absent)
  ...
  return (
    <>
      ...
      >
        ...
        <div className="card" style={{ padding: "var(--space-4)", textAlign: "center" }}>
          <div style={{ fontSize: "var(--text-2xl)", fontWeight: 800, color: "var(--color-secondary)" }}>
            {elapsed}
          </div>
          <div style={{ fontSize: "var(--text-xs)", color: "var(--color-text-3)", marginTop: 2 }}>
            Elapsed
      ...
      <div className="card" style={{ padding: "var(--space-4) var(--space-5)", marginBottom: "var(--space-5)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "var(--space-2)" }}>
          <span style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--color-text-2)" }}>
            Attendance
          </span>
          <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--color-text)" }}>
            {checkins}/{total}{" "}
            <span style={{ fontWeight: 400, color: "var(--color-text-3)" }}>({pct}%)</span>
          </span>
        </div>
        <div
          style={{
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            {entry.checkedInAt && <span>· {fmtTime(entry.checkedInAt)}</span>}
            {entry.geoVerified && (
